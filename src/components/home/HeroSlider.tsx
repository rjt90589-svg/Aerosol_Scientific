...
import { ChevronLeft, ChevronRight, FlaskConical, Wrench, Building2, Headphones, Users } from 'lucide-react'
...
const slides = [
  {
    ...
    subtitle: 'Genuine chromatography consumables and spares for various brands, collaborating with worldwide partners.',
  ...
  },
  {
    ...
    subtitle: 'Analytical, Life Sciences and General Laboratory equipment from global OEM providers.',
  ...
  },
  {
    ...
    subtitle: 'From concept and design to full execution — furniture, equipment, and installations in a compliance environment.',
  ...
  },
  {
    ...
    subtitle: 'Maintenance, training, calibration, and technical support to ensure reliable performance of your instruments.',
  ...
  },
  {
    ...
    subtitle: 'Expert multivendor support for Agilent, Waters, Shimadzu, Sciex instruments at competitive costs.',
...
export default function HeroSlider() {
  ...
  return (
    <section className="relative overflow-hidden">
          ...
          {slides.map((slide) => {
            ...
            return (
              <div key={slide.id} className="flex-[0_0_100%] min-w-0">
                <div className={`relative bg-gradient-to-br ${slide.gradient} min-h-[85vh] flex items-center`}>
                  {/* Background pattern */}
                  <div className="absolute inset-0 sci-grid-bg opacity-20" />
                  <div className="absolute inset-0 dot-pattern opacity-30" />

                  {/* Animated orbs */}
                  <div className="absolute top-20 right-20 w-72 h-72 rounded-full blur-3xl opacity-15"
                    style={{ background: slide.accent }} />
                  <div className="absolute bottom-20 left-10 w-48 h-48 rounded-full blur-2xl opacity-10"
                    style={{ background: slide.accent }} />

                  {/* Floating molecules decoration */}
                  <div className="absolute right-8 top-1/2 -translate-y-1/2 hidden xl:block opacity-10">
                    <svg viewBox="0 0 300 300" width="300" height="300">
                      <circle cx="150" cy="150" r="80" fill="none" stroke="white" strokeWidth="1" strokeDasharray="4 8" />
                      <circle cx="150" cy="150" r="140" fill="none" stroke="white" strokeWidth="0.5" strokeDasharray="2 12" />
                  ...
                  <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-5">
                        <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 py-1.5">
                          ...
                          <span className="text-xs font-bold uppercase tracking-[0.15em]" style={{ color: slide.accent }}>
                            {slide.eyebrow}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        <Link
                          ...
                          style={{ background: `linear-gradient(135deg, ${slide.accent}30, ${slide.accent}60)`, border: `1px solid ${slide.accent}60` }}
                        >
                          {slide.cta.label}
                      ...
                      <div className="flex flex-wrap gap-6 mt-10 pt-8 border-t border-white/10">
                        {[
                          ...
                          <div key={stat.label}>
                            <div className="text-2xl font-bold" style={{ color: slide.accent }}>{stat.value}</div>
                    ...
                    >
                      <div className="relative w-64 h-64">
                        <div className="absolute inset-0 rounded-full blur-3xl opacity-20" style={{ background: slide.accent }} />
      ...
      {/* Controls */}
      <button onClick={scrollPrev} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 text-white flex items-center justify-center transition-all hover:scale-110 z-10">
        <ChevronLeft size={18} />
      </button>
      <button onClick={scrollNext} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 text-white flex items-center justify-center transition-all hover:scale-110 z-10">
